test_that("Schema gen from file", {
  ...
  schema <- generateSqlSchema(csvFilepath = "settings/testSchemaDef.yaml", sqlOutputPath = tfile)
  checkmate::expect_file_exists(tfile)

  schemaDetails <- loadResultsDataModelSpecifications("settings/testSchemaDef.yaml")
  ...
  for (table in schemaDetails$tableName |> unique()) {
    ns <- schemaDetails$namespace[schemaDetails$tableName == table][1]
    dbTable <- paste0(ns, "_", table)
